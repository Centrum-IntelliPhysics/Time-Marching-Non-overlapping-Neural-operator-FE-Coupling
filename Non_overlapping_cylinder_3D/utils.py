...
def plot_uy(u, V2, foldername, title):
    ...
    grid["uy"] = u.x.array.reshape((geometry.shape[0], 3))[:, 1]  ## only y component
    # Add the grid to the plotter
    #actor_0 = p.add_mesh(grid, style="wireframe", color="k")
    
    warped = grid.warp_by_vector("u", factor=0) # deformed mesh by vector factor=1 (100%) 
    #actor_1 = p.add_mesh(warped, show_edges=True)
    actor_1 = p.add_mesh(warped, scalars= 'uy', cmap='seismic', show_edges= False) # the values are uy (y component)    
    p.remove_scalar_bar()
...
def plot_ux(u, V2, foldername):
    ...
    # Attach vector values to grid and warp grid by vector
    grid["u"] = u.x.array.reshape((geometry.shape[0], 3))
    grid["ux"] = u.x.array.reshape((geometry.shape[0], 3))[:, 0]  ## only x component
    # Add the grid to the plotter
    #actor_0 = p.add_mesh(grid, style="wireframe", color="k")
    
    warped = grid.warp_by_vector("u", factor=0) # deformed mesh by vector factor=1 (100%) 
    #actor_1 = p.add_mesh(warped, show_edges=True)
    actor_1 = p.add_mesh(warped, scalars= 'ux', cmap='seismic', show_edges= False) # the values are ux (x component)    
    p.remove_scalar_bar()
    ## scalars for add values on the mesh
    p.add_scalar_bar(
    vertical=True,
...
def plot_u_NO(u, V2, foldername): 
    #=============================
    # the figures for Neural Operator
    #=============================
    ...
    grid["u"] = u
    #actor_1 = p.add_mesh(warped, show_edges=True)
    actor_1 = p.add_mesh(grid, scalars= 'u', cmap='seismic', show_edges= False) # the values are u (x component)    
    p.remove_scalar_bar()
    ## scalars for add values on the mesh
    p.add_scalar_bar(
    vertical=True,
